...
for pair in $POST_DATA; do
    ...
    case "$key" in
        ...
        id_dev)
            uci set ds18b20.@ds18b20_s7[0].id_sensor="$val"
            ;;
        hster)
            uci set ds18b20.@ds18b20_s7[0].hister="$val"
            ;;
        pwut)
            uci set ds18b20.@ds18b20_s7[0].pwout="$val"
            ;;
        flder)
            uci set ds18b20.@ds18b20_s7[0].folder="$val"
            ;;

        # Збереження та комміт для Narodmon
        narmon9)
            uci set ds18b20.@ds18b20_s7[0].enab_nar="$val"
            uci commit ds18b20
            echo "<br>narodmon - Sensor 7 Save OK."
            ;;

        # Загальний комміт для 7-го датчика
        commit9)
            uci commit ds18b20
            echo "<br>Sensor 7 Save OK."
            ;;
        ...
        *)
            if [ -n "$key" ]; then
                uci set ds18b20.@ds18b20_s7[0]."$key"="$val"
            fi
            ;;
    esac
    IFS='&'
