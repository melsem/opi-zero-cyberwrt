...
for pair in $POST_DATA; do
    ...
    case "$key" in
        ...
        id_dev)
            uci set ds18b20.@ds18b20_s2[0].id_sensor="$val"
            ;;
        hster)
            uci set ds18b20.@ds18b20_s2[0].hister="$val"
            ;;
        pwut)
            uci set ds18b20.@ds18b20_s2[0].pwout="$val"
            ;;
        flder)
            uci set ds18b20.@ds18b20_s2[0].folder="$val"
            ;;

        # Збереження та комміт для Narodmon
        narmon2)
            uci set ds18b20.@ds18b20_s2[0].enab_nar="$val"
            uci commit ds18b20
            echo "<br>narodmon - Sensor 2 Save OK."
            ;;

        # Загальний комміт для 2-го датчика
        commit2)
            uci commit ds18b20
            echo "<br>Sensor 2 Save OK."
            ;;
        ...
        *)
            if [ -n "$key" ]; then
                uci set ds18b20.@ds18b20_s2[0]."$key"="$val"
            fi
            ;;
    esac
    IFS='&'
