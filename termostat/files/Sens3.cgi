...
for pair in $POST_DATA; do
    ...
    case "$key" in
        ...
        id_dev)
            uci set ds18b20.@ds18b20_s3[0].id_sensor="$val"
            ;;
        hster)
            uci set ds18b20.@ds18b20_s3[0].hister="$val"
            ;;
        pwut)
            uci set ds18b20.@ds18b20_s3[0].pwout="$val"
            ;;
        flder)
            uci set ds18b20.@ds18b20_s3[0].folder="$val"
            ;;

        # Збереження та комміт для Narodmon
        narmon3)
            uci set ds18b20.@ds18b20_s3[0].enab_nar="$val"
            uci commit ds18b20
            echo "<br>narodmon - Sensor 3 Save OK."
            ;;

        # Загальний комміт для 3-го датчика
        commit3)
            uci commit ds18b20
            echo "<br>Sensor 3 Save OK."
            ;;
        ...
        *)
            if [ -n "$key" ]; then
                uci set ds18b20.@ds18b20_s3[0]."$key"="$val"
            fi
            ;;
    esac
    IFS='&'
