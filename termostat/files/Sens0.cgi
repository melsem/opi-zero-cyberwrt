...
for pair in $POST_DATA; do
    ...
    case "$key" in
        ...
        id_dev)
            uci set ds18b20.@ds18b20_s0[0].id_sensor="$val"
            ;;
        hster)
            uci set ds18b20.@ds18b20_s0[0].hister="$val"
            ;;
        pwut)
            uci set ds18b20.@ds18b20_s0[0].pwout="$val"
            ;;
        flder)
            uci set ds18b20.@ds18b20_s0[0].folder="$val"
            ;;

        # Збереження та комміт для Narodmon
        narmon0)
            uci set ds18b20.@ds18b20_s0[0].enab_nar="$val"
            uci commit ds18b20
            echo "<br>narodmon - Sensor 0 Save OK."
            ;;

        # Загальний комміт для 0-го датчика
        commit0)
            uci commit ds18b20
            echo "<br>Sensor 0 Save OK."
            ;;
        ...
        *)
            if [ -n "$key" ]; then
                uci set ds18b20.@ds18b20_s0[0]."$key"="$val"
            fi
            ;;
    esac
    IFS='&'
